 ...
 *  Windows schannel 偶发 HTTP/2 握手失败时，清空目标并改用 HTTP/1.1 重试一次。 */
async function cloneGitRepo(url, dest) {
  try {
    ...
    await runGit(["-c", "http.version=HTTP/1.1", "clone", "--depth", "1", url, dest]);
  }
}
...
export async function importFromGitRepo(url, subdir, log, options = {}) {
  ...
  try {
    ...
    // 解析技能目录：显式子目录优先；留空时自动识别仓库内的 skills/ 子目录，否则退回仓库根。
    let skillsDir = tempRoot;
    if (sub !== "") {
      skillsDir = resolve(tempRoot, sub);
    } else {
      const skillsSub = resolve(tempRoot, "skills");
      const subStat = await lstatOrNull(skillsSub);
      if (subStat && subStat.isDirectory()) skillsDir = skillsSub;
    }
...
async function remoteDefaultBranch(url, cwd) {
  ...
  const symref = await runGitResult(["ls-remote", "--symref", url, "HEAD"], cwd);
  const match = /ref:\s+refs\/heads\/(\S+)\s+HEAD/.exec(symref.stdout);
  return match && match[1] ? match[1] : "main";
}

/**
 * 浅克隆（带重试）：Windows schannel 偶发 HTTP/2 握手失败
 * （"failed to receive handshake, SSL/TLS connection failed"）。
 * 首次失败后清空目标并改用 HTTP/1.1 重试一次；返回成功的那次或首次失败结果。
 */
async function cloneRepoShallow(url, dest) {
  const attempt = (extra) => runGitResult([...extra, "clone", "--depth", "1", url, dest]);
  const first = await attempt(["-c", "core.autocrlf=false"]);
  if (first.status === 0) return first;
  await fs.rm(dest, { recursive: true, force: true }).catch(() => undefined);
  await fs.mkdir(dest, { recursive: true });
  const retry = await attempt(["-c", "http.version=HTTP/1.1", "-c", "core.autocrlf=false"]);
  return retry.status === 0 ? retry : first;
}
...
export async function pushSkillToSource(root, name, options = {}) {
  ...
  try {
    ...
    let pushed = await runGitResult(["push", pushUrl, `HEAD:${branch}`], tempRoot);
    if (pushed.status !== 0) {
      // schannel HTTP/2 握手偶发失败：改用 HTTP/1.1 重试一次
      const retriedPush = await runGitResult(["-c", "http.version=HTTP/1.1", "push", pushUrl, `HEAD:${branch}`], tempRoot);
      if (retriedPush.status === 0) pushed = retriedPush;
    }
    if (pushed.status !== 0) {
