...
export async function pushSkillToSource(root, name, options = {}) {
  ...
  try {
    const cloned = await runGitResult(["-c", "core.autocrlf=false", "clone", "--depth", "1", url, tempRoot]);
    ...
    const status = await runGitResult(["status", "--porcelain", "--", relativeDest], tempRoot);
    ...
    const staged = await runGitResult(["add", "--", relativeDest], tempRoot);
    ...
    const identityArgs = identity.status !== 0 || identity.stdout.trim() === ""
      ...
      : [];
    const commit = await runGitResult([...identityArgs, "commit", "-m", `feat(skills): 更新 ${name}（由 DSH 技能管理推送）`, "--", relativeDest], tempRoot);
    ...
    const pushed = await runGitResult(["push", pushUrl, `HEAD:${branch}`], tempRoot);
    if (pushed.status !== 0) {
