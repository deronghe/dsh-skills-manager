...
/** 规整一个导入仓库条目，缺失字段回落默认值。 */
function sanitizeRepo(entry) {
  const repo = entry && typeof entry === "object" ? entry : {};
  return {
    id: typeof repo.id === "string" ? repo.id : "",
    url: typeof repo.url === "string" ? repo.url : "",
    subdir: typeof repo.subdir === "string" ? repo.subdir : "",
    label: typeof repo.label === "string" ? repo.label : "",
    skillCount: typeof repo.skillCount === "number" && repo.skillCount >= 0 ? repo.skillCount : null,
  };
}
...
/** 一次性迁移：旧版单仓库 gitRepo 尚无 repos 列表时，把它并入导入仓库列表。 */
async function migrateRepos(settings) {
  if (Array.isArray(settings.repos)) return;
  settings.repos = [];
  if (settings.gitRepo && typeof settings.gitRepo.url === "string" && settings.gitRepo.url !== "") {
    settings.repos.push({ id: randomUUID(), url: settings.gitRepo.url, subdir: typeof settings.gitRepo.subdir === "string" ? settings.gitRepo.subdir : "", label: "" });
  }
  await saveSettings(settings);
}

/** 按仓库 id 解析导入来源（url/subdir）；id 为空取首个，未命中回落到推送仓库，均无返回 null。 */
async function resolveRepoById(repoId) {
  const settings = await loadSettings();
  await migrateRepos(settings);
  const repos = Array.isArray(settings.repos) ? settings.repos : [];
  const id = String(repoId == null ? "" : repoId);
  let entry;
  if (id === "") entry = repos[0];
  else entry = repos.find((repo) => repo && repo.id === id);
  if (entry) return { id: entry.id, url: entry.url, subdir: typeof entry.subdir === "string" ? entry.subdir : "" };
  const pushRepo = settings.gitRepo;
  if (pushRepo && typeof pushRepo.url === "string" && pushRepo.url !== "") {
    return { id: "", url: pushRepo.url, subdir: typeof pushRepo.subdir === "string" ? pushRepo.subdir : "" };
  }
  return null;
}
...
export async function listRepos() {
  const settings = await loadSettings();
  await migrateRepos(settings);
  return { repos: (Array.isArray(settings.repos) ? settings.repos : []).map(sanitizeRepo) };
}

/** 新增导入仓库；url+subdir 去重，label 可选。 */
export async function addRepo(url, subdir, label) {
  const normalizedUrl = normalizeRepoUrl(url);
  if (!normalizedUrl.ok) return normalizedUrl;
  const normalizedSub = normalizeSubdir(subdir);
  if (!normalizedSub.ok) return normalizedSub;
  ...
  const dup = repos.find((repo) => repo.url === normalizedUrl.url && (typeof repo.subdir === "string" ? repo.subdir : "") === normalizedSub.subdir);
  if (dup) {
    return { ok: false, error: "该仓库已在列表中", code: "error.repo.duplicate", params: { url: normalizedUrl.url } };
  }
  const entry = { id: randomUUID(), url: normalizedUrl.url, subdir: normalizedSub.subdir, label: String(label == null ? "" : label).trim().slice(0, 64) };
  repos.push(entry);
  await saveSettings(settings);
  return { repo: sanitizeRepo(entry) };
}

/** 按 id 删除导入仓库；只删除记录，不触碰已安装的技能。 */
export async function removeRepo(id) {
  const settings = await loadSettings();
  await migrateRepos(settings);
  const repos = Array.isArray(settings.repos) ? settings.repos : [];
  const index = repos.findIndex((repo) => repo && repo.id === id);
  if (index === -1) return { ok: false, error: "仓库不存在", code: "error.repo.notFound", params: { id: String(id == null ? "" : id) } };
  repos.splice(index, 1);
  await saveSettings(settings);
  return { repos: repos.map(sanitizeRepo) };
}
...
/** 列出指定（或首个）导入仓库内的全部 skill，并回写该仓库的技能计数缓存。 */
export async function listSavedRepoSkills(repoId) {
  const repo = await resolveRepoById(repoId);
  if (repo === null) return { ok: false, error: "尚未保存 GitHub 仓库", code: "error.github.notSet" };
  const result = await listRepoSkills(repo.url, repo.subdir);
  if (result.ok !== false && repo.id !== "") {
    const settings = await loadSettings();
    const repos = Array.isArray(settings.repos) ? settings.repos : [];
    const entry = repos.find((item) => item && item.id === repo.id);
    if (entry) {
      entry.skillCount = (result.skills || []).length;
      await saveSettings(settings);
    }
  }
  return Object.assign({ repoId: repo.id }, result);
}
...
/** 从已保存（或指定）的导入仓库导入单个 skill；未保存仓库返回 notSet。options.repo 为仓库 id。 */
export async function importSavedRepoSkill(name, log, options = {}) {
  const repo = await resolveRepoById(options.repo);
  if (repo === null) return { ok: false, error: "尚未保存 GitHub 仓库", code: "error.github.notSet" };
  return importRepoSkill(repo.url, repo.subdir, name, log, options);
}

/**
 * 从来源仓库一次导入多个 skill（只克隆一次）。
 * options.update 为 true 时覆盖同名；否则跳过同名。
 * 返回 { imported, skipped, failed }；克隆/校验失败返回 { ok:false }。
 */
export async function importRepoSkills(url, subdir, names, log, options = {}) {
  const normalizedUrl = normalizeRepoUrl(url);
  if (!normalizedUrl.ok) return normalizedUrl;
  const normalizedSub = normalizeSubdir(subdir);
  if (!normalizedSub.ok) return normalizedSub;
  ...
  const wanted = Array.isArray(names)
    ? [...new Set(names.map((name) => String(name == null ? "" : name).trim()).filter(Boolean))].slice(0, 200)
    : [];
  ...
  try {
    ...
    for (const name of wanted) {
      ...
      if (!target) {
        failed.push({ name, error: `仓库中不存在该技能: ${name}`, code: "error.github.skillNotFound", params: { name } });
        continue;
      }
      const result = await importSkill(repoEntrySource(target), log, { conflict });
      if (result && result.ok === false) {
        failed.push({ name, error: result.error || `导入失败: ${name}`, code: result.code || "error.import.failed", params: result.params || { name } });
      } else {
        for (const item of result.imported || []) imported.push(item);
...
/** 从已保存（或指定）的导入仓库一次导入多个 skill；未保存仓库返回 notSet。options.repo 为仓库 id。 */
export async function importSavedRepoSkills(names, log, options = {}) {
  const repo = await resolveRepoById(options.repo);
  if (repo === null) return { ok: false, error: "尚未保存 GitHub 仓库", code: "error.github.notSet" };
  return importRepoSkills(repo.url, repo.subdir, names, log, options);
}
