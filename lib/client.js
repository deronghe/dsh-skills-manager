  ...
  factory: (require) => {
    ...
    var DICT = {
      zh: {
        ...
        "repo.manage.title": "仓库管理",
        "repo.manage.desc": "管理要导入的多个来源仓库；点击某个仓库查看并筛选其技能。",
        "repo.manage.empty": "尚未添加仓库。先在上方添加一个 GitHub 仓库。",
        "repo.manage.label": "名称（可选）",
        "repo.manage.label.placeholder": "例如 work-skills",
        "btn.repo.add": "添加仓库",
        "btn.repo.remove": "删除",
        "repo.manage.skillCount": "{count} 个 skill",
        "repo.manage.uncounted": "未统计",
        "repo.added": "已添加仓库：{name}",
        "repo.removed": "已删除仓库：{name}",
        "repo.push.title": "推送来源仓库",
        ...
        "error.repo.duplicate": "该仓库已在列表中: {url}",
        "error.repo.notFound": "仓库不存在: {id}",
        "root.dsh": "DSH 技能",
      ...
      en: {
        ...
        "repo.manage.title": "Repository management",
        "repo.manage.desc": "Manage the source repositories to import from; click a repository to view and filter its skills.",
        "repo.manage.empty": "No repositories yet. Add a GitHub repository above.",
        "repo.manage.label": "Name (optional)",
        "repo.manage.label.placeholder": "e.g. work-skills",
        "btn.repo.add": "Add repository",
        "btn.repo.remove": "Remove",
        "repo.manage.skillCount": "{count} skills",
        "repo.manage.uncounted": "not counted",
        "repo.added": "Added repository: {name}",
        "repo.removed": "Removed repository: {name}",
        "repo.push.title": "Push source repository",
        ...
        "error.repo.duplicate": "Repository already in the list: {url}",
        "error.repo.notFound": "Repository not found: {id}",
        "root.dsh": "DSH skills",
...
`;
    ...
    function SkillManagerSection(props) {
      ...
      var selectedRepoState = react.useState("");
      var selectedRepoId = selectedRepoState[0];
      var setSelectedRepoId = selectedRepoState[1];
      var repoAddFormState = react.useState({ url: "", subdir: "", label: "" });
      var repoAddForm = repoAddFormState[0];
      var setRepoAddForm = repoAddFormState[1];
      var repoManageMsgState = react.useState(null);
      var repoManageMsg = repoManageMsgState[0];
      var setRepoManageMsg = repoManageMsgState[1];
      ...
      function repoDisplayName(repo) {
        if (repo && repo.label) return repo.label;
        var m = repo && repo.url ? /github\.com\/([^/]+)\/([^/]+)/i.exec(repo.url) : null;
        if (m) return m[1] + "/" + m[2];
        return repo ? repo.url : "";
      }

      function loadRepos() {
        callApi("/repos").then(function (data) {
          var list = (data && data.repos) || [];
          setRepos({ loading: false, error: null, data: list });
          setSelectedRepoId(function (prev) {
            if (prev !== "" && list.some(function (r) { return r.id === prev; })) return prev;
            return list.length ? list[0].id : "";
          });
        }).catch(function (error) {
          setRepos(function (prev) { return { loading: false, error: error, data: prev && prev.data ? prev.data : [] }; });
        });
      }
      ...
      function addRepoClient() {
        if (busy || inflightRef.current) return;
        var url = repoAddForm.url.trim();
        if (url === "") {
          setRepoManageMsg({ ok: false, text: t("error.github.invalidUrl", { url: "" }) });
          return;
        }
        inflightRef.current = true;
        setBusy("repo-add");
        setRepoManageMsg(null);
        callApi("/repos/add", { method: "POST", headers: MUTATION_HEADERS, body: JSON.stringify({ url: url, subdir: repoAddForm.subdir, label: repoAddForm.label }) }).then(function (data) {
          inflightRef.current = false;
          setBusy(null);
          setRepoAddForm({ url: "", subdir: "", label: "" });
          setRepoManageMsg({ ok: true, text: t("repo.added", { name: repoDisplayName(data.repo) }) });
          setSelectedRepoId(data.repo.id);
          loadRepos();
        }).catch(function (error) {
          inflightRef.current = false;
          setBusy(null);
          setRepoManageMsg({ ok: false, text: translateError(t, error) });
        });
      }

      function removeRepoClient(repo) {
        if (busy || inflightRef.current) return;
        inflightRef.current = true;
        setBusy("repo-remove");
        setRepoManageMsg(null);
        callApi("/repos/remove", { method: "POST", headers: MUTATION_HEADERS, body: JSON.stringify({ id: repo.id }) }).then(function () {
          inflightRef.current = false;
          setBusy(null);
          setRepoManageMsg({ ok: true, text: t("repo.removed", { name: repoDisplayName(repo) }) });
          setSelectedRepoId(function (prev) { return prev === repo.id ? "" : prev; });
          loadRepos();
        }).catch(function (error) {
          inflightRef.current = false;
          setBusy(null);
      ...
      function loadRepoSkills(repoId) {
        setRepoList(function (prev) { return { loading: true, error: null, data: prev && prev.data ? prev.data : null }; });
        callApi("/github/skills" + (repoId ? "?repo=" + encodeURIComponent(repoId) : "")).then(function (data) {
      ...
      function importRepoSkillOne(name, installed) {
        ...
        setRepoListMsg(null);
        callApi("/github/import-one", { method: "POST", headers: MUTATION_HEADERS, body: JSON.stringify({ name: name, update: installed === true, repo: selectedRepoId }) }).then(function (data) {
          inflightRef.current = false;
          setBusy(null);
      ...
      function importRepoSelected(update) {
        ...
        setRepoListMsg(null);
        callApi("/github/import-many", { method: "POST", headers: MUTATION_HEADERS, body: JSON.stringify({ names: repoSelected, update: update === true, repo: selectedRepoId }) }).then(function (data) {
          inflightRef.current = false;
          setBusy(null);
      ...
      function repoListToolbar(skills) {
        ...
        return h("div", { className: "dssm-repo-toolbar" },
          ...
          h("button", { className: "dssm-btn dssm-btn-secondary", disabled: !!busy || !!(repoList && repoList.loading), onClick: function () { loadRepoSkills(selectedRepoId); } }, repoList && repoList.loading ? t("loading") : t("btn.repo.list.refresh")));
      }

      function selectedRepo() {
        var list = (repos && repos.data) || [];
        for (var i = 0; i < list.length; i++) if (list[i].id === selectedRepoId) return list[i];
        return null;
      }
      ...
      function pushRepoListCard() {
        ...
        if (repo) headChildren.push(h("span", { className: "dssm-count" }, repoDisplayName(repo)));
      ...
      function repoRow(repo) {
        var selected = repo.id === selectedRepoId;
        return h("div", { key: repo.id, className: "dssm-row" + (selected ? " dssm-row-selected" : ""), style: { cursor: "pointer" }, onClick: function () { selectRepo(repo.id); } },
          h("div", { className: "dssm-row-main" },
            h("div", { className: "dssm-row-id" },
              h("span", { className: "dssm-row-name" }, repoDisplayName(repo)),
              selected ? h("span", { className: "dssm-tag dssm-tag-on" }, t("status.selected")) : null),
            h("div", { className: "dssm-note", title: repo.url }, repo.url + (repo.subdir ? " · " + repo.subdir : ""))),
          h("span", { className: "dssm-count" }, repo.skillCount == null ? t("repo.manage.uncounted") : t("repo.manage.skillCount", { count: repo.skillCount })),
          h("button", { className: "dssm-btn dssm-btn-danger", disabled: !!busy, onClick: function (event) { event.stopPropagation(); removeRepoClient(repo); } }, t("btn.repo.remove")));
      }

      function pushRepoManageCard() {
        var list = (repos && repos.data) || [];
        var children = [
          h("p", { key: "desc", className: "dssm-desc", style: { padding: "0 16px 4px" } }, t("repo.manage.desc")),
          h("div", { key: "add-form", className: "dssm-form" },
            h("div", { className: "dssm-form-row" },
              h("label", { className: "dssm-label", htmlFor: "dssm-repo-add-url" }, t("repo.url")),
              h("input", { id: "dssm-repo-add-url", className: "dssm-control", type: "text", value: repoAddForm.url, placeholder: t("repo.url.placeholder"), autoComplete: "off", spellCheck: false, onChange: function (event) { setRepoAddForm({ url: event.currentTarget.value, subdir: repoAddForm.subdir, label: repoAddForm.label }); } })),
            h("div", { className: "dssm-form-row" },
              h("label", { className: "dssm-label", htmlFor: "dssm-repo-add-subdir" }, t("repo.subdir")),
              h("input", { id: "dssm-repo-add-subdir", className: "dssm-control", type: "text", value: repoAddForm.subdir, autoComplete: "off", spellCheck: false, onChange: function (event) { setRepoAddForm({ url: repoAddForm.url, subdir: event.currentTarget.value, label: repoAddForm.label }); } })),
            h("div", { className: "dssm-form-row" },
              h("label", { className: "dssm-label", htmlFor: "dssm-repo-add-label" }, t("repo.manage.label")),
              h("input", { id: "dssm-repo-add-label", className: "dssm-control", type: "text", value: repoAddForm.label, placeholder: t("repo.manage.label.placeholder"), autoComplete: "off", spellCheck: false, onChange: function (event) { setRepoAddForm({ url: repoAddForm.url, subdir: repoAddForm.subdir, label: event.currentTarget.value }); } })),
            h("div", { className: "dssm-form-actions" },
              h("button", { className: "dssm-btn", disabled: !!busy, onClick: addRepoClient }, t("btn.repo.add")),
              repoManageMsg ? h("span", { className: repoManageMsg.ok ? "dssm-status-ok" : "dssm-error", role: "status" }, repoManageMsg.text) : null)),
        ];
        if (repos && repos.error) children.push(h("div", { key: "error", className: "dssm-error", role: "alert" }, t("error.action", { error: translateError(t, repos.error) })));
        else if (!repos || (repos.loading && !repos.data)) children.push(h("p", { key: "loading", className: "dssm-empty" }, t("loading")));
        else if (!list.length) children.push(h("p", { key: "empty", className: "dssm-empty" }, t("repo.manage.empty")));
        else for (var i = 0; i < list.length; i++) children.push(repoRow(list[i]));
        nodes.push(h("div", { key: "repo-manage-card", className: "dssm-group" },
          h("div", { className: "dssm-group-head" }, h("h3", { className: "dssm-group-title" }, t("repo.manage.title")), h("span", { className: "dssm-count" }, t("summary.group", { count: list.length }))),
          children));
      }

      function pushPushCard() {
        var repoConfigured = !!(config && config.gitRepo && config.gitRepo.url);
        nodes.push(h("div", { key: "repo-push-card", className: "dssm-group" },
          h("div", { className: "dssm-group-head" }, h("h3", { className: "dssm-group-title" }, t("repo.push.title"))),
