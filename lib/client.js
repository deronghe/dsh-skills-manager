...
`;
    ...
    function SkillManagerSection(props) {
      ...
      var folderMsgState = react.useState(null);
      var folderMsg = folderMsgState[0];
      var setFolderMsg = folderMsgState[1];
      ...
      function runFolderAction(action) {
        ...
        if (action === "pick") {
          var picker = desktopDirectoryPicker() || pickDirectory;
          if (!picker) {
            setFolderMsg({ ok: false, text: t("error.action", { error: t("dir.service.missing") }) });
            return;
          }
          inflightRef.current = true;
          setBusy("folder-pick");
          setFolderMsg(null);
          picker().then(function (path) {
            if (!path) { inflightRef.current = false; setBusy(null); return null; }
            return callApi("/folder/save", { method: "POST", headers: MUTATION_HEADERS, body: JSON.stringify({ source: path }) }).then(function () {
              refresh();
              return path;
            });
          }).then(function (path) {
            inflightRef.current = false;
            setBusy(null);
            if (path) setFolderMsg({ ok: true, text: t("folder.saved.path", { path: path }) });
          }).catch(function (error) {
            inflightRef.current = false;
            setBusy(null);
            setFolderMsg({ ok: false, text: translateError(t, error) });
          });
          return;
        }
        inflightRef.current = true;
        setBusy("folder-" + action);
        setFolderMsg(null);
        var endpoint = action === "refresh" ? "/folder/refresh" : "/folder/clear";
        callApi(endpoint, { method: "POST", headers: MUTATION_HEADERS, body: JSON.stringify({}) }).then(function (data) {
          inflightRef.current = false;
          setBusy(null);
          refresh();
          if (action === "clear") { setFolderMsg({ ok: true, text: t("folder.cleared") }); return; }
          var names = ((data && data.imported) || []).map(function (item) { return item.name; });
          if (names.length) setFolderMsg({ ok: true, text: t("folder.done", { names: names.join(t("sep.names")) }) });
          else if (data && (data.failed || []).length) setFolderMsg({ ok: false, text: translateError(t, data) });
          else setFolderMsg({ ok: true, text: t("folder.empty") });
        }).catch(function (error) {
          inflightRef.current = false;
          setBusy(null);
          setFolderMsg({ ok: false, text: translateError(t, error) });
      ...
      function repoSkillRow(item) {
        var statusTag = item.installed
          ? h("span", { className: "dssm-tag " + (item.enabled ? "dssm-tag-on" : "dssm-tag-off") }, item.enabled ? t("status.enabled") : (item.localValid ? t("status.disabled") : t("status.invalid")))
          : null;
      ...
      function repoListContent() {
        ...
        return skills.map(repoSkillRow);
      }
      ...
      function pushRepoListCard() {
        ...
        if (repoList && repoList.data) headChildren.push(h("span", { className: "dssm-count" }, t("summary.group", { count: (repoList.data.skills || []).length })));
        var bodyChildren = [];
        if (!repoUrl) {
          bodyChildren.push(h("p", { className: "dssm-desc", style: { padding: "0 16px 8px" } }, t("github.list.unset")));
        } else {
          bodyChildren.push(h("div", { className: "dssm-form-actions" },
            h("span", { className: "dssm-field-hint", title: repoUrl }, repoUrl),
      ...
      function pushFolderCard() {
        var sourceFolder = snapshot.data && snapshot.data.sourceFolder ? snapshot.data.sourceFolder : "";
        nodes.push(h("div", { key: "folder-card", className: "dssm-group" },
          h("div", { className: "dssm-group-head" }, h("h3", { className: "dssm-group-title" }, t("folder.card.title"))),
          h("p", { className: "dssm-desc", style: { padding: "0 16px 4px" } }, t("folder.card.desc")),
          h("div", { className: "dssm-form" },
            h("div", { className: "dssm-form-actions" },
              h("span", { className: "dssm-field-hint", title: sourceFolder || "" }, sourceFolder ? t("folder.saved.path", { path: sourceFolder }) : t("folder.none")),
              h("button", { className: "dssm-btn dssm-btn-secondary", disabled: !!busy, onClick: function () { runFolderAction("pick"); } }, t("btn.folder.save")),
              h("button", { className: "dssm-btn dssm-btn-secondary", disabled: !!busy || !sourceFolder, onClick: function () { runFolderAction("refresh"); } }, t("btn.folder.refresh")),
              h("button", { className: "dssm-btn dssm-btn-danger", disabled: !!busy || !sourceFolder, onClick: function () { runFolderAction("clear"); } }, t("btn.folder.clear"))),
            folderMsg ? h("div", { className: "dssm-form-actions" }, statusNote(folderMsg)) : null)));
      }

      function pushGitHubCard() {
        var repo = config && config.gitRepo && config.gitRepo.url ? config.gitRepo.url + (config.gitRepo.subdir ? " · " + config.gitRepo.subdir : "") : "";
        nodes.push(h("div", { key: "github-card", className: "dssm-group" },
          h("div", { className: "dssm-group-head" }, h("h3", { className: "dssm-group-title" }, t("github.card.title"))),
          h("p", { className: "dssm-desc", style: { padding: "0 16px 4px" } }, t("github.card.desc")),
          h("div", { className: "dssm-form" },
            h("div", { className: "dssm-form-actions" },
              h("span", { className: "dssm-field-hint", title: repo || "" }, repo || t("repo.unset")),
              h("button", { className: "dssm-btn dssm-btn-secondary", disabled: !!busy || !repo, onClick: function () { runGitRepoAction(false); } }, t("btn.github.import")),
              h("button", { className: "dssm-btn dssm-btn-secondary", disabled: !!busy || !repo, onClick: function () { runGitRepoAction(true); } }, t("btn.github.update")),
              h("button", { className: "dssm-btn dssm-btn-danger", disabled: !!busy || !repo, onClick: clearGitRepoClient }, t("btn.github.clear"))),
      ...
      function pushRepoContent() {
        ...
        nodes.push(h("div", { key: "repo-card", className: "dssm-group" },
          ...
          config === null ? h("p", { className: "dssm-empty" }, t("loading")) : h("div", { className: "dssm-form" },
            ...
            config.gitRepo && config.gitRepo.url ? h("div", { className: "dssm-form-actions" },
              h("a", { className: "dssm-link", href: config.gitRepo.url, target: "_blank", rel: "noreferrer" }, t("repo.open")),
              h("span", { className: "dssm-field-hint" }, config.gitRepo.url + (config.gitRepo.subdir ? " · " + config.gitRepo.subdir : ""))) : h("p", { className: "dssm-field-hint" }, t("repo.empty")))));
      }
