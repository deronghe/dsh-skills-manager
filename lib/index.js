...
import { state, setSkillEnabled, deleteSkill, importSkill, pushSkillToSource, readConfig, saveConfig, pluginMeta, userRoots, logPath } from "./core.js";
      ...
      try {
        ...
        if (req.method === "GET" && path === "/api/dsh-skills-manager/config") {
          return run(res, async () => {
            const { gitRepo } = await readConfig();
            return { gitRepo };
          });
        }
        ...
        if (req.method === "HEAD") {
          ...
          if (path === "/api/dsh-skills-manager/config") {
            return run(res, async () => {
              const { gitRepo } = await readConfig();
              return { gitRepo };
            }, undefined, true);
        ...
        return enqueueMutation(() => {
          switch (path) {
            ...
            case "/api/dsh-skills-manager/config":
              return run(res, () => saveConfig(body && typeof body === "object" ? body : {}));
