...
import { state, setSkillEnabled, deleteSkill, importSkill, pushConfig, savePushConfig, pluginMeta, pushSkillToSource, saveSourceFolder, clearSourceFolder, syncSourceFolder, saveGitRepo, clearGitRepo, syncGitRepo, listSavedRepoSkills, importSavedRepoSkill, importSavedRepoSkills, userRoots, logPath } from "./core.js";
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === "GET" && path === "/api/dsh-skills-manager/github/skills") {
          return run(res, listSavedRepoSkills);
        }
        if (req.method === "HEAD") {
        ...
        return enqueueMutation(() => {
          switch (path) {
            ...
            case "/api/dsh-skills-manager/github/import-one":
              return run(res, () => importSavedRepoSkill(String(body.name || ""), log, { update: body.update === true }), afterWrite);
            case "/api/dsh-skills-manager/github/import-many":
              return run(res, () => importSavedRepoSkills(Array.isArray(body.names) ? body.names : [], log, { update: body.update === true }), afterWrite);
