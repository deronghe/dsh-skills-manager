...
// - 路由：/api/dsh-skills-manager/state | config | meta | enable | disable | delete | import | push
//        | folder/save | folder/clear | folder/refresh | github/import | github/import-one | github/import-many | github/clear | github/skills
//        | repos | repos/add | repos/remove
// - 全部接口先校验 loopback Host；写接口再校验请求标记与 JSON
// - $DSH_HOME\skills 可上传、删除、启停、推送；$DSH_AGENTS_HOME\skills 仅允许查看

import { appendFile, rename, rm, stat } from "node:fs/promises";
import { state, setSkillEnabled, deleteSkill, importSkill, pushConfig, savePushConfig, pluginMeta, pushSkillToSource, saveSourceFolder, clearSourceFolder, syncSourceFolder, saveGitRepo, clearGitRepo, syncGitRepo, listSavedRepoSkills, importSavedRepoSkill, importSavedRepoSkills, listRepos, addRepo, removeRepo, userRoots, logPath } from "./core.js";
  ...
  const route = ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === "GET" && path === "/api/dsh-skills-manager/config") {
          return run(res, pushConfig);
        }
        if (req.method === "GET" && path === "/api/dsh-skills-manager/meta") {
          return run(res, pluginMeta);
        }
        if (req.method === "GET" && path === "/api/dsh-skills-manager/repos") {
          return run(res, listRepos);
        }
        if (req.method === "GET" && path === "/api/dsh-skills-manager/github/skills") {
          return run(res, () => listSavedRepoSkills(u.searchParams.get("repo") || ""));
        }
        if (req.method === "HEAD") {
          ...
          if (path === "/api/dsh-skills-manager/config") return run(res, pushConfig, undefined, true);
        ...
        return enqueueMutation(() => {
          switch (path) {
            ...
            case "/api/dsh-skills-manager/config":
              return run(res, () => savePushConfig(body && typeof body === "object" ? body : {}));
            case "/api/dsh-skills-manager/push":
              return run(res, () => pushSkillToSource(rootByKey[String(body.root || "dsh")], String(body.name || ""), {
                dryRun: body.dryRun === true,
                log,
              }), afterWrite);
            case "/api/dsh-skills-manager/folder/save":
              return run(res, () => saveSourceFolder(String(body.source || ""), log));
            case "/api/dsh-skills-manager/folder/clear":
              return run(res, () => clearSourceFolder(log));
            case "/api/dsh-skills-manager/folder/refresh":
              return run(res, () => syncSourceFolder(log), afterWrite);
            case "/api/dsh-skills-manager/github/import":
              return run(res, async () => {
                const url = String(body.url || "").trim();
                if (url !== "") {
                  const saved = await saveGitRepo(url, String(body.subdir || ""), log);
                  if (saved.ok === false) return saved;
                }
                return syncGitRepo(log, { update: body.update === true });
              }, afterWrite);
            case "/api/dsh-skills-manager/github/import-one":
              return run(res, () => importSavedRepoSkill(String(body.name || ""), log, { update: body.update === true, repo: typeof body.repo === "string" ? body.repo : "" }), afterWrite);
            case "/api/dsh-skills-manager/github/import-many":
              return run(res, () => importSavedRepoSkills(Array.isArray(body.names) ? body.names : [], log, { update: body.update === true, repo: typeof body.repo === "string" ? body.repo : "" }), afterWrite);
            case "/api/dsh-skills-manager/github/clear":
              return run(res, () => clearGitRepo(log));
            case "/api/dsh-skills-manager/repos/add":
              return run(res, () => addRepo(String(body.url || ""), String(body.subdir || ""), String(body.label || "")));
            case "/api/dsh-skills-manager/repos/remove":
              return run(res, () => removeRepo(String(body.id || "")));
            default:
