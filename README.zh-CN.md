...
>
> 本仓库是 [`MichengAI/dsh-skills-manager`](https://github.com/MichengAI/dsh-skills-manager) 的 fork，新增 DSH Desktop 上传支持。在 Desktop（Electron）外壳中，`File.path` 不可用，且宿主的目录选择服务只提供页面内浏览能力；本 fork 通过 Desktop preload 桥（`__DSH_DESKTOP_FILE_PATH__` 与 `__DSH_DESKTOP_PICK_DIRECTORY__`）解析所选文件真实路径并打开系统原生目录选择框。
...
- 在 DSH Desktop 中可上传单个 `SKILL.md` 文件或整个插件目录；本 fork 通过 Desktop preload 桥解析真实文件路径并打开系统原生目录选择框。
- 可保存一个本地源文件夹，点击「刷新文件夹」即可扫描并把其中的 skill 批量导入 DSH（同名 skill 跳过、不覆盖）。
- 可按地址导入 GitHub（或任意 Git）远程仓库中的 skill（可指定子目录），并点击「更新」重新拉取、用最新版本覆盖同名技能。
- 在「仓库」页浏览已保存来源仓库内的全部 skill 并逐个导入：新 skill 显示「导入」，已安装的显示「更新」。
...
```text
请把 DSH 插件 @deronghe/dsh-skills-manager 最新版装进本机 web profile，使用官方 npm 源执行：dsh plugin --profile web add @deronghe/dsh-skills-manager@latest --registry=https://registry.npmjs.org/。装完执行 dsh --profile web --dump-config，确认已挂载 skills-manager，并提醒我重启 DSH Web 后硬刷新浏览器。
```

从源码安装：

```text
请从 https://github.com/deronghe/dsh-skills-manager 安装 DSH 插件：克隆仓库，执行 npm install 和 npm test，再在该目录执行 dsh plugin --profile web add .。不要只复制 lib。然后执行 dsh --profile web --dump-config，确认已挂载 skills-manager，并提醒我重启 DSH Web 后硬刷新浏览器。
```
...
| WorkBuddy | 把上面其中一句发给 WorkBuddy；源码安装也可同时粘贴仓库地址 `https://github.com/deronghe/dsh-skills-manager`。 |
...
需要钉死某一版时，把 `@latest` 换成具体版本，例如 `@0.2.0`。
...
| 保存源文件夹 | 点击「选择文件夹」保存一个本地目录，再点「刷新文件夹」扫描并导入其中的 skill（同名跳过）。 | 仅 DSH 本地技能 |
| 清除源文件夹 | 点击「清除」忘记已保存的目录；不会删除已导入的技能。 | 仅 DSH 本地技能 |
| 从 Git 仓库导入 | 输入仓库地址（可选子目录），点击「导入」克隆并导入其中的 skill。 | 仅 DSH 本地技能 |
| 从 Git 仓库更新 | 点击「更新」重新拉取已保存的仓库，并用最新版本覆盖同名技能。 | 仅 DSH 本地技能 |
| 逐个导入仓库技能 | 在「仓库」页查看已保存来源仓库的技能列表：新 skill 点击「导入」，已安装的点击「更新」即可覆盖。 | 仅 DSH 本地技能 |
